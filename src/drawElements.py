...
def draw_gameboard(canvas, board_rects, gameboard, current_piece, player):
    ...
    for row in range(board_arr.shape[0]):
        for column in range(board_arr.shape[1]):
            ...
            elif board_arr[row][column] == constants.PLAYER3_VALUE:
                pygame.draw.rect(canvas, constants.HUMAN_PARAMS["default_p3"]["color"], rect)
            elif board_arr[row][column] == constants.PLAYER4_VALUE:
                pygame.draw.rect(canvas, constants.HUMAN_PARAMS["default_p4"]["color"], rect)
            ...
            if [row, column] == constants.STARTING_PTS["player1"]:
                ...
                canvas.blit(text, [rect.x, rect.centery - 2])
            elif [row, column] == constants.STARTING_PTS["player3"]:
                text = pygame.font.SysFont(None, 15).render("Player 3", True, constants.COLORS["BLACK"])
                canvas.blit(text, [rect.x, rect.centery - 2])
            elif [row, column] == constants.STARTING_PTS["player4"]:
                text = pygame.font.SysFont(None, 15).render("Player 4", True, constants.COLORS["BLACK"])
                canvas.blit(text, [rect.x, rect.centery - 2])
            else:
...
def draw_pieces(canvas, player1, player2, player3, player4, active_player, selected):
    p1_pieces, p2_pieces, p3_pieces, p4_pieces = player1.remaining_pieces, player2.remaining_pieces, player3.remaining_pieces, player4.remaining_pieces
    p1_color, p2_color, p3_color, p4_color = player1.color, player2.color, player3.color, player4.color
    ...
    for key, val in p2_pieces.items():
        if not (key == selected and player2.number == active_player.number):
            for unit_sq in val["rects"]:
                ...
                unit_sq.x += MARGIN
                unit_sq.y += MARGIN
    for key, val in p3_pieces.items():
        if not (key == selected and player3.number == active_player.number):
            for unit_sq in val["rects"]:
                pygame.draw.rect(canvas, p3_color, unit_sq)
                unit_sq.x -= MARGIN
                unit_sq.y -= MARGIN
                pygame.draw.rect(canvas, constants.COLORS["WHITE"], unit_sq, MARGIN)
                unit_sq.x += MARGIN
                unit_sq.y += MARGIN
    for key, val in p4_pieces.items():
        if not (key == selected and player4.number == active_player.number):
            for unit_sq in val["rects"]:
                pygame.draw.rect(canvas, p4_color, unit_sq)
                unit_sq.x -= MARGIN
                unit_sq.y -= MARGIN
                pygame.draw.rect(canvas, constants.COLORS["WHITE"], unit_sq, MARGIN)
                unit_sq.x += MARGIN
...
def draw_infobox(canvas, player1, player2, player3, player4, active_player, turn_number):
    text_dict = {"p1_score": "Player 1 Score: %s" % player1.score,
                 "p2_score": "Player 2 Score: %s" % player2.score,
                 "p3_score": "Player 3 Score: %s" % player3.score,
                 "p4_score": "Player 4 Score: %s" % player4.score,
                 "title": "BLOKUS"}
    ...
    if active_player.number == 1:
        ...
        current_player_rect = pygame.Rect(
            (330, 20, piece_box_width - 40, info_box_height - 40))  # x=1010, y=20, w=290, h=60
    elif active_player.number == 3:
        current_player_rect = pygame.Rect(
            (660, 40, piece_box_width - 40, info_box_height - 40))  # x=20, y=40, w=290, h=60
    else:
        current_player_rect = pygame.Rect((piece_box_width + board_width + 20, 40, piece_box_width - 40,
                                           info_box_height - 40))  # x=1010, y=40, w=290, h=60
    if turn_number < 2:
    ...
    font_dict = {"p1_score": font.render(text_dict["p1_score"], False, constants.COLORS["WHITE"]),
                 ...
                 "p3_score": font.render(text_dict["p3_score"], False, constants.COLORS["WHITE"]),
                 "p4_score": font.render(text_dict["p4_score"], False, constants.COLORS["WHITE"]),
    ...
    pos_rect_dict = {"p1_score": pygame.Rect((0, 0, piece_box_width, info_box_height)),  # x=0, y=0, w=330, h=100
                     "p2_score": pygame.Rect((330, 0, piece_box_width, info_box_height)),
                     # x=990, y=0, w=330, h=100
                     "p3_score": pygame.Rect((660, 0, piece_box_width, info_box_height)),
                     # x=0, y=20, w=330, h=100
                     "p4_score": pygame.Rect((990, 0, piece_box_width, info_box_height)),
                     # x=990, y=20, w=330, h=100
    ...
    pos_dict = {"p1_score": pos_rect_dict["p1_score"].center,
                ...
                "p3_score": pos_rect_dict["p3_score"].center,
                "p4_score": pos_rect_dict["p4_score"].center,
...
def draw_infobox_msg(canvas, player1, player2, player3, player4, msg_key):
    game_over_text = ""
